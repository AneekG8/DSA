//given a complete BST turn it into a min heap(in-place)
...
//now again  do preorder traversal and update values from inorder array

//utility function for storing in-order traversal
...
//utility function
void heapFromBSTutil(TreeNode* root,vector<int> v,int* i)
{
    if(root)
    {
        root->data=v[*i];
        ...
        heapFromBSTutil(root->left,v,i);

        heapFromBSTutil(root->right,v,i);
    }
}

//main function for converting a BST to heap
void heapFromBST(BST* t)
{
    vector<int> inorder;

    storeInorder(t->root,&inorder);

    int i=0;

    heapFromBSTutil(t->root,inorder,&i);
}

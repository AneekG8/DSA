...
TreeNode* TreeFromInPost(int in[],int post[],int n)
{
    ...
    TreeNode* temp=new TreeNode(post[n-1]);

    //traverse left subtree

    //if left subtree is not empty
    if(root!=0)
        temp->left=TreeFromInPost(in,post,root);

    //traverse right subtree

    //if right subtree is not empty
    if(root!=n-1)
        temp->right=TreeFromInPost(in+root+1,post+root,n-root-1);
...
Tree createTree(int in[],int post[],int n)
{
    ...
    t.root=TreeFromInPost(in,post,n);

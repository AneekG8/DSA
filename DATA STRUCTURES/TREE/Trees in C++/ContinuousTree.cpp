//check if a binary tree is continuous or not
...
bool isContinuous(TreeNode* root)
{
    if(!root)
        return true;
    ...
    return (abs(root->data-root->right->data)==1) && (abs(root->data-root->left->data)==1) && isContinuous(root->left) && isContinuous(root->right);
}

bool isContinuous(Tree t)
{
    isContinuous(t.root);
}
...
int main()
{
    ...
    isContinuous(t) ? cout<<"YES"<<endl : cout<<"NO"<<endl;

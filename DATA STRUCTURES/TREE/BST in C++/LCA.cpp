//given two nodes(data) find the lowest(furthest from root) common ancestor
...
//if a node is lesser or grater than both the given nodes mean both the nodes are in same subtree
//implying that there exists a node further from root which is ancestor of both

//now if a node lies in between them means the given nodes are in opposite subtrees
//so there won't be any common ancestor existing in lower level


//space complexity O(h)
//to reduce the space complexity use iterative method

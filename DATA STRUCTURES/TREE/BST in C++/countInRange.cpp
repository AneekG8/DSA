//count the no. of nodes in Range in a BST
...
//simple postorder traversal while checking for the range
int countRange(TreeNode* root,int min,int max)
{
    if(root)
    {
        int l=0,r=0;
        ...
        if(min<root->data)
            l=countRange(root->left,min,max);

        //go to right only if max>root
        if(max>root->data)
            r=countRange(root->right,min,max);

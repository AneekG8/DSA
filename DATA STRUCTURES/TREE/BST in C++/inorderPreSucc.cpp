//find the inorder predecessor and successor in a BST
...
//inorder traversal
//store the first greater element
//store all the smaller elements up to last smaller element
void find_p_s(TreeNode* root,int key,TreeNode** pre,TreeNode** succ)
{
    if(root && !(*succ))    //once we have got succ we must have got pre so no need to recurse anymore
    {
        find_p_s(root->left,key,pre,succ);

        if(root->data>key && !(*succ))  //succ will be null until first greater element is stored
        {
            *succ=root;
            return;
        }
        ...
        find_p_s(root->right,key,pre,succ);
    }
}

void find_p_s(BST t,int key,TreeNode** pre,TreeNode** succ)
{
    *pre=nullptr;
    *succ=nullptr;

    find_p_s(t.root,key,pre,succ);
}
//previous method calls unnecessary recursive calls

void findPreSucc(TreeNode* root,int key,TreeNode** pre,TreeNode** succ)
{
    if(root)
    {
        if(key<root->data)
        ...
        {
            *pre=root;
            findPreSucc(root->right,key,pre,succ);
        }
...
//!method iterative
void findPreSuccItr(BST t,int key,TreeNode** pre,TreeNode** succ)
{
    *pre=nullptr;
    *succ=nullptr;

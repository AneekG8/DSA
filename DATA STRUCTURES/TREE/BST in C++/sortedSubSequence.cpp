//check if a sorted subsequence exists in a BST
...
//inorder traversal
//increment i upon matching from arr
void existsSubSeqUtil(TreeNode*root,int arr[],int n,int& i)
{
    if(root && i<n)
    {
        existsSubSeqUtil(root->left,arr,n,i);

        if(arr[i]==root->data)
            i++;

        existsSubSeqUtil(root->right,arr,n,i);
    }
}


//wrapping function
bool existsSubSeq(BST t,int arr[],int n)
{
    int i=0;

    existsSubSeqUtil(t.root,arr,n,i);
...
int main()
{
    ...
    int seq[] = {4, 6, 8,12,13,14};
    int n = sizeof(seq)/sizeof(seq[0]);

    existsSubSeq(t, seq, n)? cout << "Yes" :
                            cout << "No";

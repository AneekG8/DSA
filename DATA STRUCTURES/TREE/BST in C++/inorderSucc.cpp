...
//if a node has right subtree then leftmost node(minimum valued node) of right subtree is successor

//else we start searching from root
//if a node is at left of root then root is marked as its successor
//if node is at right of root then we do not have any successor

//returns leftmost node in a tree
...
TreeNode* getInorderSuccessor(BST t,int key)
{
        TreeNode* target=t.search(key);
...
//another method is to use parent pointer in the nodes**
...
int main()
{
    ...
    cout<<getInorderSuccessor(t,8)->data;

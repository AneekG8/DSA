...
void storeInorderUtil(TreeNode* root,vector<TreeNode*>& v)
{

    if(root)
    {
        storeInorderUtil(root->left,v);

        v.push_back(root);

        storeInorderUtil(root->right,v);
    }
}

//main function for storing inorder noder
vector<TreeNode*> storeInorder(Tree t)
{
    vector<TreeNode*> v;

    storeInorderUtil(t.root,v);
...
//for sorting we only swap data of the nodes

//!use any optimized sorting algo

//utility function to swap data of the nodes
...
void sortInorder(vector<TreeNode*>& v)
{
...
//main function to convert a BT to a BST

//converts the BT itself to a BST
//as well as returns the root
//so we can store it in a BST class

TreeNode* convertBTtoBST(Tree t)
{
    vector<TreeNode*> v=storeInorder(t);

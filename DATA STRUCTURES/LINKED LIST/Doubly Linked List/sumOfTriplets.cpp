...
the task is to find triplets in doubly linked list whose sum is equal to given value x, without using any extra space ?
*/
#include<iostream>
using namespace std;
...
void getTriplets(DLL* list,int sum)
{
    Node* node=list->head;
    ...
    while(node!=tail->prev)
    {
        ...
        while(start!=end && end->next!=start)
        {
            if(start->data+end->data < sum-node->data)
                start=start->next;
            else if(start->data+end->data > sum-node->data)
                end=end->prev;
            else
            {
                cout<<node->data<<"+"<<start->data<<"+"<<end->data<<endl;
                start=start->next;
                end=end->prev;
            }
        }

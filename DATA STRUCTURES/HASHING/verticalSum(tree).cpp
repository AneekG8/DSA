//given a binary tree print sum of the nodes vertically
...
//using map for sorted order(leftmost to rightmost)
void verticalSumUtil(TreeNode* root,int hd,map<int,int>& m)
{
    if(root)
    {
        m[hd]+=root->data;
...
//wrapping function
void verticalSum(Tree t)
{
    map<int,int> m;

    verticalSumUtil(t.root,0,m);
